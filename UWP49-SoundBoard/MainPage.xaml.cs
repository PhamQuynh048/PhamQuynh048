...
namespace UWP49_SoundBoard
{
    ...
    public sealed partial class MainPage : Page
    {
        ...
        private void SearchAutoSuggestBox_TextChanged(AutoSuggestBox sender, AutoSuggestBoxTextChangedEventArgs e)
        {
            ...
            SoundManager.GetAllSounds(Sounds);
            Suggestions = Sounds.Where(prop => prop.Name.StartsWith(sender.Text)).Select(p => p.Name).ToList();
            SearchAutoSuggestBox.ItemsSource = Suggestions;
        }

        private static void GetSoundsByName(ObservableCollection<Sound> sounds, string name)
        {
            var allSounds = getSounds();
            var filteredSounds = allSounds.Where(p => p.Name).ToList();
            sounds.Clear();
